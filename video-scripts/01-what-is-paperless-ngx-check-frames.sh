...
OUT="${2:-/tmp/01-what-is-paperless-ngx-contact-sheet.jpg}"
FRAME_DIR="${3:-/tmp/01-what-is-paperless-ngx-contact-frames}"
...
for t in 0 10 20 30 45 60 75 90 105 120; do
  ffmpeg \
    -y \
    -ss "$t" \
    -i "$TRACK" \
...
FILTER="${FILTER},pad=302:196:(ow-iw)/2:(oh-ih)/2,tile=5x2"
